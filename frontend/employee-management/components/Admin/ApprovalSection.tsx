...
export default function ApprovalSection ({data} : {data: Leave}) {
    ...
    return (
        <div className="space-y-3">
            ...
            <div className="flex flex-row space-x-3 justify-between w-full">
                <button className="w-1/2 text-center bg-green-500 rounded px-3 py-2 text-white hover:bg-green-600 hover:shadow-md" onClick={(e) => handleApprove(session.user.token, data.id, "Approved")}>Approve</button>
                <button className="w-1/2 text-center bg-red-500 rounded px-3 py-2 text-white hover:bg-red-600 hover:shadow-md" onClick={(e) => handleApprove(session.user.token, data.id, "Denied")}>Deny</button>
            </div>
        </div>
    );
}